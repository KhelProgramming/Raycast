...
OUT_DIR = os.path.join(ART_DIR, "optionC_personal_only")
...
# ======================================================================================
# GLOBAL MLP (only used to validate calibration)
# ======================================================================================
...
def calibrate_and_train_personal_logreg():
    ...
    session = CalibrationSession("Calibration (Option C)")
    X_list = []
    y_list = []
    per_gesture = {}
    ...
    # ✅ PERSONAL label encoder (prevents label-index mismatch)
...
# LIVE PERSONAL ONLY
# ======================================================================================
def live_personal_only():
    if not os.path.exists(PERSONAL_MODEL_PATH) or not os.path.exists(PERSONAL_LE_PATH):
        raise FileNotFoundError("Missing personal artifacts. Run: python optionC...py calibrate")
    ...
    WIN = "Live (Option C: Personal LogReg Only)"
    maximize_window(WIN)
    ...
    try:
        while True:
            ...
            if res.multi_hand_landmarks:
                f18 = extract_geometric18(res.multi_hand_landmarks[0])
                Xs = scaler.transform([f18])

                probs = personal.predict_proba(Xs)[0]
                top = int(np.argmax(probs))
                p1 = float(probs[top])
                label = personal_le.inverse_transform([top])[0]

                text = f"PERSONAL: {label} p={p1:.2f}"
                color = (0, 255, 0)
...
if __name__ == "__main__":
    ...
    if mode == "calibrate":
        ...
        print("Usage: python optionC_mlp_validator_personal_only_logreg.py [calibrate|live]")